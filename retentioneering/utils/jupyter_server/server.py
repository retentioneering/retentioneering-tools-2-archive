...
class ServerManager():
  ...
  def _create_main_listener(self):
    ...
    if (env == "colab"):
      ...
      google.colab.output.register_callback('JupyterServerMainCallback', lambda server_id, method, request_id, payload  : self._on_colab_func_called(server_id, method, request_id, payload))
    if (env == "classic"):
      get_ipython().kernel.comm_manager.register_target('JupyterServerMainCallback', lambda comm, open_msg : self._on_comm_message(comm, open_msg))
